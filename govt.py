...
def scrape_govt_portals():
    options = Options()
    options.add_argument("--headless")
    driver = webdriver.Chrome(options=options)
    
    portals = [
        {"name": "AICTE", "url": "https://internship.aicte-india.org/recentlyposted.php", "container": ".internship-card"},
        {"name": "NATS", "url": "http://www.nats.education.gov.in/notifications.php", "container": ".news-item"}
    ]
    ...
    for portal in portals:
        try:
            ...
            elements = driver.find_elements(By.CSS_SELECTOR, portal['container'])
            for el in elements:
                title = el.find_element(By.TAG_NAME, "h4").text
                link = el.find_element(By.TAG_NAME, "a").get_attribute("href")
                
                # Check for 2026 Batch Relevance
                if any(x in title.lower() for x in ["senior", "lead", "manager"]): continue

                job_doc = {
                    "jobTitle": title,
                    "companyName": f"Government of India ({portal['name']})",
                    "location": "India",
                    "applicationLink": link,
                    "jobType": "Internship/Apprentice",
                    "branch": ["Engineering"], # General tagging
                    "eligibleYear": ["2026 pursuing"],
                    "category": "Government",
                    "source": portal['name'],
                    "syncedAt": datetime.utcnow()
                }
                jobs_col.update_one({"applicationLink": link}, {"$set": job_doc}, upsert=True)
